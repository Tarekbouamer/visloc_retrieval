    ...
    def train_epoch(self):
        ...
        for step, (tuples, target) in enumerate(self.train_dl):
            ...
            for tuple_i, target_i in zip(tuples, target):
                ...
                vecs = torch.zeros(
                    num_imgs, self.cfg.body.out_dim).cuda()
            ...
            with torch.no_grad():
                ...
                self.write_metrics(metrics, step, len(
                    self.train_dl), global_step=self.global_step)
        ...
        for step, (tuples, target) in enumerate(self.val_dl):
            ...
            with torch.no_grad():
                ...
                for tuple_i, target_i in zip(tuples, target):
                    ...
                    vecs = torch.zeros(num_imgs, self.cfg.body.out_dim).cuda()
...
class ImageRetrievalTrainer(TrainerBase):
    def __init__(self, args, cfg):
        ...
        self.start_epoch = 1
        self.global_step = 0
        self.max_epochs = cfg["scheduler"].epochs
    ...
    def resume_or_load(self, resume=True):
        ...
        snapshot_last = resume_from_snapshot(
            self.model, snapshot_last_path, ["body", "head"])
    ...
    def val_epoch(self):
        if (self.epoch % self.cfg["general"].val_interval) != 0:
            return
    ...
    def test_epoch(self):
        ...
        if (self.epoch % self.cfg["general"].test_interval) != 0:
            return
    ...
    def build_model(self, cfg):
        ...
        """
        model_name = cfg.body.arch
        pretrained = cfg.body.pretrained

        model = create_retrieval(model_name, cfg, pretrained=pretrained)
    ...
    def build_ema_model(self, args, cfg):
        ...
        ema_model = utils.ModelEmaV2(
            self.model, decay=cfg.body.ema_decay)
