        ...
        for j in range(self.num_clusters):  # slower than non-looped, but lower memory usage
            residual = x.unsqueeze(0).permute(1, 0, 2, 3, 4) - \
                self.centroids[j:j + 1, :].expand(x.size(2), x.size(
                    3), -1, -1).permute(2, 3, 0, 1).unsqueeze(0)
...
class PatchNetVLADNet(RetrievalBase):
    def __init__(self, cfg):
        ...
        # pool
        self.pool = PatchNetVLAD(num_clusters=self.cfg.num_clusters,
                                 dim=self.cfg.enc_dim)

        # pca whitening
        if self.cfg.num_pcs > 0:
            pca_conv = nn.Conv2d(self.cfg.num_clusters * self.cfg.enc_dim,
                                 self.cfg.num_pcs, kernel_size=(1, 1), stride=1, padding=0)
    ...
    def forward(self, data=None, **kwargs):
        ...
        # pool
        vlad_local, vlad_global = self.pool(x)

        # pca whitening
        if hasattr(self, "WPCA"):
            vlad_global = self.WPCA(vlad_global.unsqueeze(-1).unsqueeze(-1))

        return {"features": vlad_global, "features_local": vlad_local}
...
default_cfgs = {
    'mapillary_vgg16_patchnetvlad_128':
        _cfg(drive='https://drive.google.com/uc?id=1ep7ECNaphnN5nk9EKcvkKwsGCsOpxyz8',
             backbone="vgg16", enc_dim=512, num_clusters=16, num_pcs=128),

    'mapillary_vgg16_patchnetvlad_512':
        _cfg(drive='https://drive.google.com/uc?id=1lYllv_EpBsUh--eOe7Lub4HBP5czydfA',
             backbone="vgg16", enc_dim=512, num_clusters=16, num_pcs=512),

    'mapillary_vgg16_patchnetvlad_4096':
        _cfg(drive='https://drive.google.com/uc?id=1S_VJoxDQcB-QQmFKZ0gQ8Jpsmhw_ieDI',
             backbone="vgg16", enc_dim=512, num_clusters=16, num_pcs=4096),
}
...
@register_retrieval
def mapillary_vgg16_patchnetvlad_128(cfg=None, pretrained=True, **kwargs):
    """Constructs a Patch-NetVLAD model with  Mapillary weights"""
    return _create_model('mapillary_vgg16_patchnetvlad_128', cfg, pretrained, **kwargs)


@register_retrieval
def mapillary_vgg16_patchnetvlad_512(cfg=None, pretrained=True, **kwargs):
    """Constructs a Patch-NetVLAD model with  Mapillary weights"""
    return _create_model('mapillary_vgg16_patchnetvlad_512', cfg, pretrained, **kwargs)


@register_retrieval
def mapillary_vgg16_patchnetvlad_4096(cfg=None, pretrained=True, **kwargs):
    """Constructs a Patch-NetVLAD model with  Mapillary weights"""
    return _create_model('mapillary_vgg16_patchnetvlad_4096', cfg, pretrained, **kwargs)
