...
class MSLS(Dataset):
    def __init__(self, root_dir, cities='', nNeg=5, transform=None, mode='train', task='im2im', subtask='all',
        ...
        assert (task == 'im2im' and seq_length == 1) or (
            task != 'im2im' and seq_length > 1)
        ...
        for city in self.cities:
            ...
            if self.mode in ['train', 'val']:
                # load query data
                qData = pd.read_csv(
                    join(root_dir, subdir, city, 'query', 'postprocessed.csv'), index_col=0)
                qDataRaw = pd.read_csv(
                    join(root_dir, subdir, city, 'query', 'raw.csv'), index_col=0)

                # load database data
                dbData = pd.read_csv(
                    join(root_dir, subdir, city, 'database', 'postprocessed.csv'), index_col=0)
                dbDataRaw = pd.read_csv(
                    join(root_dir, subdir, city, 'database', 'raw.csv'), index_col=0)

                # arange based on task
                qSeqKeys, qSeqIdxs = self.arange_as_seq(qData, join(
                    root_dir, subdir, city, 'query'), seq_length_q)
                ...
                if self.mode in ['val']:
                    qIdx = pd.read_csv(
                        join(root_dir, subdir, city, 'query', 'subtask_index.csv'), index_col=0)
                    dbIdx = pd.read_csv(
                        join(root_dir, subdir, city, 'database', 'subtask_index.csv'), index_col=0)

                    # find all the sequence where the center frame belongs to a subtask
                    val_frames = np.where(qIdx[self.subtask])[0]
                    qSeqKeys, qSeqIdxs = self.filter(
                        qSeqKeys, qSeqIdxs, val_frames)

                    val_frames = np.where(dbIdx[self.subtask])[0]
                    dbSeqKeys, dbSeqIdxs = self.filter(
                        dbSeqKeys, dbSeqIdxs, val_frames)

                # filter based on panorama data
                if self.exclude_panos:
                    panos_frames = np.where(
                        (qDataRaw['pano'] is False).values)[0]
                    qSeqKeys, qSeqIdxs = self.filter(
                        qSeqKeys, qSeqIdxs, panos_frames)

                    panos_frames = np.where(
                        (dbDataRaw['pano'] is False).values)[0]
                    dbSeqKeys, dbSeqIdxs = self.filter(
                        dbSeqKeys, dbSeqIdxs, panos_frames)
                ...
                def seqIdx2frameIdx(seqIdx, seqIdxs):
                    return seqIdxs[seqIdx]
                # frameIdx2seqIdx = lambda frameIdx, seqIdxs: np.where(seqIdxs == frameIdx)[0][1]

                def frameIdx2uniqFrameIdx(frameIdx, uniqFrameIdx):
                    return np.where(np.in1d(uniqFrameIdx, frameIdx))[0]

                def uniqFrameIdx2seqIdx(frameIdxs, seqIdxs):
                    return np.where(np.in1d(seqIdxs, frameIdxs).reshape(seqIdxs.shape))[0]
                ...
                for q_seq_idx in range(len(qSeqKeys)):
                    ...
                    if len(p_uniq_frame_idxs) > 0:
                        p_seq_idx = np.unique(uniqFrameIdx2seqIdx(
                        ...
                        if self.mode == 'train':
                            ...
                            n_uniq_frame_idxs = np.unique(
                                [n for nonNeg in nI[q_uniq_frame_idx] for n in nonNeg])
                            n_seq_idx = np.unique(uniqFrameIdx2seqIdx(
                                unique_dbSeqIdx[n_uniq_frame_idxs], dbSeqIdxs))
            ...
            # when GPS / UTM / pano info is not available
                ...
                # arange in sequences
                qSeqKeys, qSeqIdxs = self.arange_as_seq(qIdx, join(
                    root_dir, subdir, city, 'query'), seq_length_q)
        ...
        if mode == 'train':
            ...
            self.negCache = np.asarray(
                [np.empty((0,), dtype=int)] * len(self.qIdx))
    ...
    def __calcSamplingWeights__(self):
        ...
        print(
            "#Sideways [{}/{}]; #Night; [{}/{}]".format(len(self.sideways), N, len(self.night), N))
        print("Forward and Day weighted with {:.4f}".format(1))
        if len(self.night) != 0:
            print("Forward and Night weighted with {:.4f}".format(
                1 + N / len(self.night)))
        if len(self.sideways) != 0:
            print("Sideways and Day weighted with {:.4f}".format(
                1 + N / len(self.sideways)))
        if len(self.sideways) != 0 and len(self.night) != 0:
            print("Sideways and Night weighted with {:.4f}".format(
                1 + N / len(self.night) + N / len(self.sideways)))
    ...
    def collate_fn(batch):
        ...
        query = dataloader.default_collate(query)
        positive = dataloader.default_collate(positive)
        negCounts = dataloader.default_collate(
    ...
    def update_subcache(self, net=None, outputdim=None):
        ...
        if net is None:
            qidxs = np.random.choice(
                len(self.qIdx), self.cached_queries, replace=False)
        ...
        opt = {'batch_size': self.bs, 'shuffle': False,
               'num_workers': self.threads, 'pin_memory': True}
        qloader = torch.utils.dataloader(ImagesFromList(
            self.qImages[qidxs], transform=self.transform), **opt)
        ploader = torch.utils.dataloader(ImagesFromList(
            self.dbImages[pidxs], transform=self.transform), **opt)
        nloader = torch.utils.dataloader(ImagesFromList(
            self.dbImages[nidxs], transform=self.transform), **opt)
