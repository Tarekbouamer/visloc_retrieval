...
def ParisOxfordTestDataset(root_dir, name=None):
    ...
    if name == "val_eccv20":
        ...
        meta['img_names'] = [cid2filename(
            db['cids'][i], ims_root) for i in range(len(db['cids']))]
        ...
        meta['img_names'] = [
            path.join(meta['images_path'], item + meta['_ext']) for item in db['imlist']]
        meta['query_names'] = [
            path.join(meta['images_path'], item + meta['_ext']) for item in db['qimlist']]
        meta['gnd'] = db['gnd']
