...
class SatDataset(data.Dataset):
    ...
    def create_epoch_tuples(self,  cfg, model):
        ...
        logger.info('Creating tuples for an epoch of {%s}--{%s}', self.name, self.mode)
        ...
        # Dataloaders
        q_dl     = data.DataLoader( ImagesFromList(root='',  images=[self.qImages[i] for i in q_indices],   transform=tf), **dl_opt)
        p_dl     = data.DataLoader( ImagesFromList(root='',  images=[self.images[i]  for i in p_indices],   transform=tf), **dl_opt)
        n_dl     = data.DataLoader( ImagesFromList(root='',  images=[self.images[i]  for i in n_indices],   transform=tf), **dl_opt)
        ...
        with torch.no_grad():
            ...
            qvecs = torch.zeros(len(q_dl), global_cfg.getint("global_dim")).cuda()
            ...
            pvecs = torch.zeros(len(p_dl), global_cfg.getint("global_dim")).cuda()
            ...
            nvecs = torch.zeros(len(n_dl), global_cfg.getint("global_dim")).cuda()
