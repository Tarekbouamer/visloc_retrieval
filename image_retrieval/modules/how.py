...
from core.utils.PCA import PCA_whitenlearn_shrinkage
...
class SmoothingAvgPooling(nn.Module):
    ...
    def forward(self, x):
        return functional.smoothing_avg_pooling(x, kernel_size=self.kernel_size)      


class DimReduction(nn.Conv2d):
    """
    ...
    def initialize_pca_whitening(self, des):
        ...
        """
        m, P = PCA_whitenlearn_shrinkage(des)
        m, P = m.T, P.T
...
def smoothing_avg_pooling(feats, kernel_size):
    ...
    return functional.avg_pool2d( feats, 
...
def weighted_spoc(ms_feats, ms_weights):
    ...
    return functional.normalize(desc)
