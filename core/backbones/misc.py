...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # added Drop Path for fast training  before addition
        x = residual + self.drop_path(self.convs(x))

        if self.convs.bn1.activation == "relu":
            return functional.relu(x, inplace=True)
        
        elif self.convs.bn1.activation == "silu":
              return functional.silu(x, inplace=True)
          
        elif self.convs.bn1.activation == "h_swish":
            return functional.hardswish(x, inplace=True)
        
        elif self.convs.bn1.activation == "gelu":
