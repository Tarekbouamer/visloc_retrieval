...
from ..linalg import cross_product_matrix, eye_like, vec_like
from .projection import projection_from_KRt, depth
...
def decompose_essential_matrix(E_mat):
    """
    ...
    W = cross_product_matrix(torch.tensor([[0., 0., 1.]]).type_as(E_mat))
    W[..., 2, 2] += 1.
...
def essential_from_Rt(R1, t1, R2, t2):
    """
...
def motion_from_essential(E):
    ...
    """
    assert len(E.shape) >= 2 and E.shape[-2:], E.shape
...
def motion_from_essential_choose_solution(E_mat, K1, K2, x1, x2, mask=None):
    """
        ...
        3d points. Internally uses decompose_essential_matrix and then chooses
        the best solution based on the combination that gives more 3d points in front of the camera plane from triangulate_points
    """
    assert len(E_mat.shape) >= 2 and E_mat.shape[-2:], E_mat.shape
    assert len(K1.shape) >= 2 and K1.shape[-2:] == (3, 3), K1.shape
    assert len(K2.shape) >= 2 and K2.shape[-2:] == (3, 3), K2.shape
