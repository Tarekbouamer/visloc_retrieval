...
from image_retrieval.modules.losses             import TripletLoss
from image_retrieval.modules.heads.how_head     import HowHead
...
def computer_PCA_layer(model, train_dataloader, config, varargs):
    ...
    # create layer
    layer = nn.Conv2d(inp_dim, out_dim, (1, 1), padding=0, bias=True)
    
    projection = torch.Tensor(P[:layer.weight.shape[0], :]).unsqueeze(-1).unsqueeze(-1)
...
def make_model(args, config, train_dataloader, **varargs):
    ...
    global_head = HowHead(inp_dim=body_dim,
                          out_dim=output_dim,
