...
def make_parser():
    # ArgumentParser
    parser = argparse.ArgumentParser(description='Prepare Image Google Landmark 2018')


    parser.add_argument('--data', metavar='EXPORT_DIR',
                        help='path to train / val GL19 dataste')
...
def main(args):
    ...
    # extra
    train_list  = [cid for cid in train_list_all  if os.path.exists(os.path.join(args.data, 'train', cid + '.jpg'))  ]
    val_list    = [cid for cid in val_list_all    if os.path.exists(os.path.join(args.data, 'train', cid + '.jpg'))  ]
    ...
    for mode in ['train', 'val']:
        ...
        for i, image in enumerate(tqdm(image_list)):
            ...
            # not a list     
            pidxs = np.random.choice(pidxs_potential, min(len(pidxs_potential), 1))
