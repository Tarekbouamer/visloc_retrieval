...
def make_model(args, config, train_dataloader, **varargs):
    ...
    global_head = TransformerHead(  inp_dim=body_dim,
                                    out_dim=output_dim,
                                    num_encs = ir_config.getint("num_encs"),
...
def make_optimizer(model, config, epoch_length):
    ...
    for k, v in model.ret_head.named_children():
        ...
        if k.find("encoder") != -1:
            encs_params += [p for p in v.parameters() if p.requires_grad]
    ...
    # Transformer head
    assert len(net_norm_params) + len(net_other_params) + len(pool_params) + len(whiten_params) + len(encs_params) \
    ...
    parameters = [
        ...
        {
            "params": encs_params,
            "lr":           LR * lr_coefs["encoders"],
            "weight_decay": WEIGHT_DECAY * weight_decay_coefs["encoders"]
        }
    ]
