...
from core.utils.sequence    import pad_packed_images
...
from image_retrieval.datasets.tuples    import TuplesDataset, TuplesTransform, collate_tuples, iss_collate_fn,  ParisOxfordTestDataset
...
def make_optimizer(model, config, epoch_length):
    ...
    pool_params     = []
    whiten_params   = []
    for k, v in model.ret_head.named_modules():
        if k.find("pool") != -1:
    ...
    assert len(net_norm_params) + len(net_other_params) + len(pool_params) + len(whiten_params) == len([p for p in model.parameters() if p.requires_grad]), \
...
def train(model, config, dataloader, optimizer, scheduler, meters, **varargs):
    ...
    for it, batch in enumerate(dataloader):
        ...
        for _, (q, p, ns, target, neg_nums) in enumerate(zip(batch["q"], batch["p"], batch["ns"], batch["target"], batch["neg_nums"])):
            ...
            # Pack and pad images
            tuple   = model._prepare_inputs(q, p, ns)
            tuple, _ = pad_packed_images(tuple)
            
            # To device
            tuple   = tuple.to(device=varargs["device"])
            target  = target.to(device=varargs["device"])
            
            # Run model
            vecs =  model(img=tuple, do_loss=True, do_whitening=True)
                        
            # compute loss
            loss = model.ret_algo.loss(vecs["ret_pred"], target)
            loss.backward()
